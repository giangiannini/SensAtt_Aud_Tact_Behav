...
OG_intensity = 3; %mA
n = 8; %amount of comparisons for each stimulation to the OG_intensity. 
max_distance = 20; %percent
session = 1; 
...
if isfile(strcat(subj_folder, '\new_intensities.mat'))
    ...
    intensities = linspace(OG_intensity*(1-max_distance/100), OG_intensity*(1+max_distance/100),9);
end

iti = 800 + (1000-800) .* rand(n*9,1); %generate n*7 random numbers between 800 and 1000.
...
for i = 1:size(Log.trials,1)
    ...
    stop(ao);
    queueOutputData(ao, stim_wave*OG_intensity);
...
%     vbl = Screen('Flip', window, vbl + ((Log.trials(i,2)/1000/ifi - 1) - 0.5) * ifi);
    ...
    ao.startBackground();      
    stop(ao);
    queueOutputData(ao, stim_wave*Log.trials(i,1));
...
save(strcat("Log_session_", int2str(session),"_tactile.mat"), "Log");
...
if session == 1
    [value, pos_max] = min(abs(Q-0.98));
    max_asyntote = t(pos_max); 
    [value, pos_min] = min(abs(Q-0.02));
    min_asyntote = t(pos_min); 

    new_intensities_tactile = linspace(min_asyntote, max_asyntote, 9); 
    display(new_intensities_tactile)
    save(strcat(subj_folder, "/new_intensities_tactile.mat"), "new_intensities_tactile")

    figure; plot(output_summary.Intensity, hits, 'o');
    hold on; plot(t, Q);
    xlabel('Intensities')
    ylabel('Hit Proportion')
    xline(new_intensities_tactile)
    exportgraphics(gca, strcat("psychometric_curve", int2str(session) ,"_tactile.png"))
    close all
else
    figure; plot(output_summary.Intensity, hits, 'o');
    hold on; plot(t, Q);
